...
"""
This module contains dataclasses for a workout. 

Firestore database:
- Workout is the main document for a workout
- Tracks are saved in a subcollection under the workout document.


COMMON PROPERTIES that are used for each workout, track and segment:
{
    start_time: date,
    end_time: date,
    ...
    stopped_time: number, (seconds)
    avg: number, (m/s)
    max_speed: number, (m/s)
    uphill: number, (meters)
    downhill: number, (meters)
...
WORKOUT:
{
    ...
    description: string,
    date_added: date
    [COMMON PROPERTIES FOR A WORKOUT]
}
...
TRACK (in tracks subcollection of workout):
{
    ...
    [COMMON PROPERTIES FOR A TRACK]
    SEGMENTs: [{
        segment_number: 1,   
        [COMMON PROPERTES FOR A SEGMENT]
        coordinates: [GeoPoint, ...],
        elevations: [...], (meters)
        times_from_start: [...], (seconds)
        distances_from_start: [...], (meters)
        speeds: [...]     (not implemented)
        },
        {
            ...
        },
...
# NOTE: Some gpxpy-methods return None and some 0 if data not found, for example
# get_uphill_downhill -> (0,0,) but get_elevation_extremes -> (None, None)

# moving_data could be better to calculate from a smoothed version of gpx like suggested in the gpxpy docs

# gpxpy does not seem to provide effiant way to calculate properties for all gpx-parts (segments, tracks, gpx)
# efficiently. Could re-use the segment properties to calculate track properties and so on. Now segment properties are
...
# an instance variable, an instance variable set in __post_init__ will (probably) not be included in the dict.
